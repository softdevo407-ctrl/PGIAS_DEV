...
const UserRoleAssignmentPage = () => {
  ...
  const handleAdd = async () => {
    // Fetch centres data when clicking Assign Role for new centre codes
    try {
      const centresData = await centresAPI.getAll();
      setCentres(Array.isArray(centresData) ? centresData : []);
      // fetch available screens/pages
      try {
        const screens = await screensAPI.getAll();
        setScreensList(Array.isArray(screens) ? screens : []);
        setScreenAccess({});
      } catch (err) {
  ...
  const handleEdit = (assignment) => {
    ...
    setValidationErrors({});
    setShowModal(true);
    // For edit mode, try to load screens for this user-role
    (async () => {
      try {
        const screens = await screensAPI.getAll();
        setScreensList(Array.isArray(screens) ? screens : []);
        // load existing assignments for this user-role
        try {
          const assigned = await userRoleAPI.getById(assignment.loginId, assignment.roleCode, assignment.centreCode);
          // assigned may not include screens; fetch via userroles screens endpoint
          const assignedScreens = await userRoleAPI.getScreensByUserRole(assignment.loginId, assignment.roleCode, assignment.centreCode);
          const map = {};
          if (Array.isArray(assignedScreens)) {
            assignedScreens.forEach(s => {
              map[s.screenCode] = s.accessType || 'READ';
            });
          }
          setScreenAccess(map);
        } catch (e) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setError('');
      // build payload including selected screens
      const payload = { ...formData };
      const screensPayload = Object.entries(screenAccess)
        .filter(([code, access]) => access && access !== 'NONE')
        .map(([code, access]) => {
          const s = screensList.find(x => x.screenId === code || x.screenId === code);
          return {
            screenCode: code,
            screenName: s ? s.screenName : code,
            accessType: access,
            status: 'ACTIVE',
      ...
      if (screensPayload.length > 0) payload.screens = screensPayload;
